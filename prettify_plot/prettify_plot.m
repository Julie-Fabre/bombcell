...
for iColorBar = 1:length(colorbars)
    ...
    if currColorbar.Limits(1) < 0 && currColorbar.Limits(2) > 0 % equalize, and use a diverging colormap 
        if max(abs(currColorbar.Limits)) > 4
            currColorbar.Limits = [-ceil(max(abs(currColorbar.Limits))), ...
                ceil(max(abs(currColorbar.Limits)))];
        else % don't round up
             currColorbar.Limits = [-max(abs(currColorbar.Limits)), ...
        ...
        if max(abs(currColorbar.Limits)) > 4
            currColorbar.Limits = [min(0, ceil(max(currColorbar.Limits))),...
                max(0, ceil(max(currColorbar.Limits)))];
        else % don't round up
              currColorbar.Limits = [min(0, max(currColorbar.Limits)),...
