...
def get_default_parameters(
    ...
    param = {
        # Quality metric display and caclulation paramaters
        ## General running and saving paramaters
        ...
        "save_mat_file_for_GUI": False,  # TODO use scipy to save .mat file?
        ...
        "sp_decay_lin_fit": False, # if false use an exponential fit (preferred). if true, use linear fit
        ...
        # Quality metric classification thresholds
        ...
        "min_main_peak_to_trough_ratio": 5, # trough should be min 5 x bigger than 1rst peak to count as non-somatic 
        "min_width_main_trough": 5,  # in samples
        "non_somatic_trough_peak_ratio": 0.8,
        "non_somatic_peak_before_to_after_ratio": 3, #if units have an initial peak before the trough,
    ...
    if ephys_meta_dir is not None:
        ...
        param["extract_raw_waveforms"] = True
        if gain_to_uV is None or np.isnan(gain_to_uV):
        ...
        param["gain_to_uV"] = gain_to_uV
        param["extract_raw_waveforms"] = False
...
def default_parameters_for_unitmatch(
   ...
   param = get_default_parameters(kilosort_path, raw_dir, kilosort_version, ephys_meta_dir, gain_to_uV)
   
   # Waveform extraction parameters for unit match: 
   param["extract_raw_waveforms"] = True
   param["decompress_data"] = True
   param["save_multiple_raw"] = True
   param["detrend_waveform"]= False 
