...
param = qm.checkParameterFields(param);

% get unit max channels
maxChannels = qm.helpers.getWaveformMaxChannel(templateWaveforms);

% extract and save or load in raw waveforms
[rawWaveformsFull, rawWaveformsPeakChan, signalToNoiseRatio] = qm.helpers.extractRawWaveformsFast(param, ...
...
[uniqueTemplates, ~, spikeTimes_samples, spikeTemplates, templateAmplitudes, ...
    ...
    qMetric.maxChannels] = ...
    qm.removeDuplicateSpikes(spikeTimes_samples, spikeTemplates, templateAmplitudes, ...
    pcFeatures, rawWaveformsFull, rawWaveformsPeakChan, signalToNoiseRatio, ...
...
for iUnit = 1:size(uniqueTemplates, 1)
    ...
    [percentageSpikesMissing_gaussian, percentageSpikesMissing_symmetric, ksTest_pValue, ~, ~, ~] = ...
        qm.percSpikesMissing(theseAmplis, theseSpikeTimes, timeChunks, param.plotDetails);

    %% fraction contamination (false positives)
    tauR_window = param.tauR_valuesMin:param.tauR_valuesStep:param.tauR_valuesMax;
    [fractionRPVs, ~, ~] = qm.fractionRPviolations(theseSpikeTimes, theseAmplis, ...
    ...
    [theseSpikeTimes, theseAmplis, theseSpikeTemplates, qMetric.useTheseTimesStart(iUnit), qMetric.useTheseTimesStop(iUnit), ...
        qMetric.RPV_tauR_estimate(iUnit)] = qm.defineTimechunksToKeep( ...
    ...
    [qMetric.percentageSpikesMissing_gaussian(iUnit), qMetric.percentageSpikesMissing_symmetric(iUnit), ...
        ...
        forGUI.ampliGaussianFit{iUnit}] = qm.percSpikesMissing(theseAmplis, theseSpikeTimes, ...
        thisUnits_timesToUse, param.plotDetails);

    [qMetric.fractionRPVs(iUnit, :), ~, ~] = qm.fractionRPviolations(theseSpikeTimes, theseAmplis, ...
    ...
    [qMetric.presenceRatio(iUnit)] = qm.presenceRatio(theseSpikeTimes, theseAmplis, param.presenceRatioBinSize, ...
        qMetric.useTheseTimesStart(iUnit), qMetric.useTheseTimesStop(iUnit), param.plotDetails);

    %% maximum cumulative drift estimate
    [qMetric.maxDriftEstimate(iUnit), qMetric.cumDriftEstimate(iUnit)] = qm.maxDriftEstimate(pcFeatures, pcFeatureIdx, theseSpikeTemplates, ...
        theseSpikeTimes, channelPositions(:, 2), thisUnit, param.driftBinSize, param.computeDrift, param.plotDetails);

    %% number spikes
    qMetric.nSpikes(iUnit) = qm.numberSpikes(theseSpikeTimes);
    ...
    [qMetric.nPeaks(iUnit), qMetric.nTroughs(iUnit), qMetric.isSomatic(iUnit), forGUI.peakLocs{iUnit}, ...
        ...
        forGUI.tempWv(iUnit, :)] = qm.waveformShape(templateWaveforms, thisUnit, qMetric.maxChannels(thisUnit), ...
    ...
    %% amplitude
    if param.extractRaw
        qMetric.rawAmplitude(iUnit) = qm.getRawAmplitude(rawWaveformsFull(iUnit, rawWaveformsPeakChan(iUnit), :), ...
    ...
    if param.computeDistanceMetrics
        [qMetric.isoD(iUnit), qMetric.Lratio(iUnit), qMetric.silhouetteScore(iUnit), ...
            forGUI.unit_mahal_counts{iUnit}, forGUI.unit_mahal_edges{iUnit}, forGUI.noise_mahal_counts{iUnit}, forGUI.unit_mahal_edges{iUnit}] = qm.getDistanceMetrics(pcFeatures, ...
...
qMetric = qm.saveQMetrics(param, qMetric, forGUI, savePath);
fprintf('\n Saved quality metrics from %s to %s \n', param.rawFile, savePath)

unitType = qm.getQualityUnitType(param, qMetric, savePath);
qm.plotGlobalQualityMetric(qMetric, param, unitType, uniqueTemplates, forGUI.tempWv);
end
