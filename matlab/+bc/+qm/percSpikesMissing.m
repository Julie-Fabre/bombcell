...
for iTimeChunk = 1:numel(timeChunks) - 1
    ...
    [spikeCountsPerAmpliBin, bins] = histcounts(theseAmplitudes_here, nBins);
    ...
    % First part: Create descending sequence
    start_val = bins(maxAmpli_val_smooth) - bin_steps;
    num_steps = floor(size(surrogate_amplitudes, 2)/2);
    descending_seq = start_val:-bin_steps:(start_val - bin_steps*(num_steps-1));
    remaining_bins = bins(maxAmpli_val(1):end);

    % Combine both parts
    surrogate_bins = [fliplr(descending_seq), remaining_bins];
...
if param.plotDetails
    ...
    for iTimeChunk = 1:length(timeChunks)
        line([timeChunks(iTimeChunk), timeChunks(iTimeChunk)], ...
         ...
         %dd outlier line and label
        outlier_line = quantiles(2) + iqr_threshold*iqr;
        yline(outlier_line, 'Color', [0.5430, 0, 0.5430], 'LineWidth', 1.5)
        text([xlims(2), xlims(2)] * [0.98; 0.02], [outlier_line, outlier_line] * [1.0; 0.91], 'Outlier Threshold', ...
            'HorizontalAlignment', 'right', 'VerticalAlignment', 'bottom', ...
            'Color', [0.5430, 0, 0.5430], 'FontWeight', 'bold')
