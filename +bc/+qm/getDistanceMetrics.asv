...
otherFeaturesInd = nan(0, size(pc_features, 2), param.nChannelsIsoDist);
otherFeatures = nan(0, size(pc_features, 2), param.nChannelsIsoDist);
...
for iID = 1:numel(uniqueIDs)
    ...
    if any(ismember(theseChannels, currentChannels))
        ...
        nonMemberChannels = ~ismember(theseChannels, currentChannels);
        if any(nonMemberChannels) % replace by NaNs
            otherFeatures(nCount:nCount+size(channelSpikes, 1)-1, :, nonMemberChannels) = nan(length(nCount:nCount+size(channelSpikes, 1)-1), 3, sum(nonMemberChannels));
            otherFeaturesInd(nCount:nCount+size(channelSpikes, 1)-1, :, nonMemberChannels) = nan(length(nCount:nCount+size(channelSpikes, 1)-1), 3, sum(nonMemberChannels));
        end
    end
end
