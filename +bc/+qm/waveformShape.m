...
if any(isnan(thisWaveform)) % kilosort can sometimes return all NaNs in a waveform, we classify these units as noise
    ...
    [PKS, peakLocs] = findpeaks(squeeze(thisWaveform), 'MinPeakProminence', minProminence);
    ...
    troughLoc = troughLocs(TRS == max(TRS));
    troughLoc = troughLoc(1);

    % Get the main peak location
    peakLoc = peakLocs(PKS == max(PKS));
    maxPK = max(PKS);
    ...
    if peakLoc < troughLoc
        ...
        if maxPK < (possible_realPeak * firstPeakRatio)
            if any(possible_peakLoc + troughLoc - 1> troughLocs)
