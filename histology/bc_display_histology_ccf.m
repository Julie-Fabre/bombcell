function bc_display_histology_ccf(tv,av,st,slice_im_path)
...
if size(slice_im_fn,2) == 2 % rocksaw: two large files with all slices inside: one for red channel, one for green channel 
    ...
    gui_data.slice_im = cell(floor(nImages/3),1);
    greenChannel = loadtiff(slice_im_fn{2});
    for curr_slice = 1:floor(nImages/3)
        currIdx = (curr_slice )*3 -2;
        gui_data.slice_im{curr_slice} = greenChannel(:,:,currIdx);
    end
...
gui_data.curr_histology_slice = 10;
gui_data.histology_im_h = imagesc(gui_data.slice_im{end-gui_data.curr_histology_slice},'Parent',gui_data.histology_ax);
colormap(gray)
...
[~,cam_plane] = max(abs(cam_vector./norm(cam_vector)));
    ...
    % coordinates
        ...
        plane_ap = ...
            (0*plane_ml+0*plane_dv + plane_offset)/ ...
            -(-1);
...
tv_slice = squeeze(gui_data.tv(gui_data.curr_histology_slice,1:slice_px_space:size(gui_data.tv,2),1:slice_px_space:size(gui_data.tv,3)));
av_slice = squeeze(gui_data.av(gui_data.curr_histology_slice,1:slice_px_space:size(gui_data.tv,2),1:slice_px_space:size(gui_data.tv,3)));
end
