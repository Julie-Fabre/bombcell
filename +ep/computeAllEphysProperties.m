...
[rawWaveformsFull, rawWaveformsPeakChan, signalToNoiseRatio] = qm.helpers.extractRawWaveformsFast(paramEP, ...
...
for iUnit = 1:length(uniqueTemplates)
    ...
    ephysProperties.acg(iUnit, :) = ep.computeACG(theseSpikeTimes, paramEP.ACGbinSize, paramEP.ACGduration, paramEP.plotThis);

    [ephysProperties.postSpikeSuppression_ms(iUnit), ephysProperties.tauRise_ms(iUnit), ephysProperties.tauDecay_ms(iUnit),...
        ephysProperties.refractoryPeriod_ms(iUnit)] = ep.computeACGprop(ephysProperties.acg(iUnit, :), paramEP.ACGbinSize, paramEP.ACGduration);
    
    %% ISI-based properties
    ISIs = diff(spikeTimes);

    [ephysProperties.propLongISI(iUnit), ephysProperties.coefficient_variation(iUnit),...
         ephysProperties.coefficient_variation2(iUnit),  ephysProperties.isi_skewness(iUnit)] = ep.computeISIprop(ISIs, theseSpikeTimes);
    ...
    [ephysProperties.waveformDuration_peakTrough_us(iUnit), ephysProperties.halfWidth_ms(iUnit), ...
        ...
        ep.computeWaveformProp(templateWaveforms,thisUnit, ephysProperties.maxChannels(thisUnit),...
    ...
    [ephysProperties.mean_firingRate(iUnit), ephysProperties.fanoFactor(iUnit),...
        ephysProperties.max_FiringRate(iUnit), ephysProperties.min_FiringRate(iUnit)] = ep.computeSpikeProp(theseSpikeTimes);
...
try
    ephysProperties = ep.saveEphysProperties(paramEP, ephysProperties, savePath);
