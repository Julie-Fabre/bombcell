function [rawWaveformsFull, rawWaveformsPeakChan, spikeMap] = bc_extractRawWaveformsFast(param, spikeTimes_samples,...
...
if any(strfind(spikeFile.name,'cbin'))
...
elseif ~isempty(old_rawWaveformFolder) && reExtract == 0
    ...
    %         memmapfile(fullfile(spikeFile.folder, spikeFile.name),'Format',{'int16',[nChannels,n_samples],'data'});
    %     catch
    ...
    if UsePython
        ...
        for bid = 1:length(batchidx)
            ...
            try
                ...
                tmpdata = pyrunfile("Ephys_Reader_FromMatlab.py","chunk",...
                    datapath = strrep(fullfile(spikeFile.folder,spikeFile.name),'\','/'),start_time=batchidx{bid}(1),end_time=endidx); %0-indexed!!
        ...
        fid = fopen(fullfile(spikeFile.folder, spikeFile.name), 'r');
