function rawWaveforms = bc_extractRawWaveformsFast(nChannels, nSpikesToExtract, spikeTimes, spikeTemplates, rawFolder, plotThis)
...
for iCluster = 1:nClust
    ...
    [~, rawWaveforms(iCluster).peakChan] =  max(max(abs(spkMapMean_sm), [], 2), [], 1); 
     ...
     clf;
     for iSpike = 1:10
        plot(spikeMap(1,:,iSpike)); hold on;
     end
     clf;
     plot(rawWaveforms(iCluster).spkMapMean(1,:)); hold on;
    if (mod(iCluster, 20) == 0 || iCluster == nClust) && plotThis
