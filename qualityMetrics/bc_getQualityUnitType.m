...
if param.computeDistanceMetrics && ~isnan(param.isoDmin)
    ...
    unitType(qMetric.nPeaks > param.maxNPeaks | qMetric.nTroughs > param.maxNTroughs | qMetric.isSomatic ~= param.somatic ...
        ...
        qMetric.waveformDuration_peakTrough > param.maxWvDuration | qMetric.waveformBaselineFlatness >= param.maxWvBaselineFraction) = 0; % NOISE or NON-SOMATIC
     unitType(qMetric.percentageSpikesMissing_gaussian <= param.maxPercSpikesMissing & qMetric.nSpikes > param.minNumSpikes & ...
        qMetric.fractionRPVs(:,qMetric.RPV_tauR_estimate) <= param.maxRPVviolations & ...
