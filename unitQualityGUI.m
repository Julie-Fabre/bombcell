...
function unitQualityGUI(memMapData, ephysData, qMetrics, param, probeLocation, goodUnits, plotRaw)
...
iChunk = 1;
initializePlot(unitQualityGuiHandle, ephysData, qMetrics, goodUnits, uniqueTemps, plotRaw)
updateUnit(unitQualityGuiHandle, memMapData, ephysData, iCluster, qMetrics, param, ...
    probeLocation, goodUnits, uniqueTemps, iChunk, plotRaw);
...
function initializePlot(unitQualityGuiHandle, ephysData, qMetrics, goodUnits, uniqueTemps, plotRaw)
...
legend('this cluster', 'other clusters');
isoDTitle = title('');
...
if goodUnits(iCluster) == 1
    set(guiData.mainTitle, 'String', ['Unit ', num2str(iCluster), ', good'], 'Color', [0, .5, 0]);
else
    set(guiData.mainTitle, 'String', ['Unit ', num2str(iCluster), ', bad'], 'Color', [1, 0, 0]);
end
...
if qMetrics.rawAmplitude(iCluster) < param.minAmplitude
    set(guiData.rawTitle, 'String', '\color[rgb]{1 0 0}Mean raw waveform');
else
...
if qMetrics.Fp(iCluster) > param.maxRPVviolations
    set(guiData.acgTitle, 'String', '\color[rgb]{1 0 0}ACG');
else
...
if qMetrics.Fp(iCluster) > param.maxRPVviolations
    set(guiData.isiTitle, 'String', '\color[rgb]{1 0 0}ISI');
else
...
set(guiData.currIsoD, 'XData', qMetrics.Xplot{iCluster}(:, 1), 'YData', qMetrics.Xplot{iCluster}(:, 2)) % Scatter plot with points of size 10
...
if qMetrics.percSpikesMissing(iCluster) > param.maxPercSpikesMissing
    set(guiData.ampliFitTitle, 'String', '\color[rgb]{1 0 0}% spikes missing');
else
